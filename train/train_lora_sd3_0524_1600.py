...
def main(train_configs):
    ...
    for epoch in range(real_num_train_epochs):
        ...
        for step, batch in enumerate(progress_bar):
            ...
            # 使用均匀分布生成timesteps（0到1之间）
            timesteps = torch.rand((bsz,), device=device)
            # 将timesteps扩展为噪声调度器期望的格式
            sigmas = timesteps.float()
            timesteps_scaled = (timesteps * 1000).long()  # 缩放到调度器期望的范围

            # Flow matching的噪声添加公式
            t = sigmas.reshape(-1, 1, 1, 1)
            # 线性插值生成带噪声的latents
            noisy_latents = ((1.0 - t) * model_input + t * noise).to(torch.float16)
            target = (noise - model_input).to(torch.float16)
